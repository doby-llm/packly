...
fun ListDetailScreen(doc: PacklyAppDocument, listId: ListId, onBack: () -> Unit, onToggle: (ItemId) -> Unit) {
    ...
    Scaffold(topBar = { TopAppBar(title = { Text(list?.name ?: "List") }, navigationIcon = { IconButton(onClick = onBack) { Icon(Icons.Rounded.ArrowBack, contentDescription = "Back") } }) }) { padding ->
